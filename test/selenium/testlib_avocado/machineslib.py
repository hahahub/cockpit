...
class MachinesLib(SeleniumTest):
    ...
    def create_vm(self, name, graphics='vnc', ptyconsole=False, state='running', wait=False):
        ...
        img = "{}/s390xmini.qcow2".format(SYS_POOL_PATH)
        pool_name = "default"
    ...
    def wait_vm_complete_start(self, vmargs):
        ...
        if log_file is not None:
            wait(
                lambda: "login:" in self.machine.execute("sudo tail -n 1 {0}".format(log_file)),
                delay=5)
        else:
            sleep(10)
    ...
    def create_vm_by_ui(self,
                        ...
                        name='default',
                        source_type='file',
                        source='/var/lib/libvirt/images/ppc64lemini.qcow2',
    ...
    def check_vm_reboot(self, vm_name, log_file):
        ...
        wait(lambda: 'login:' in self.machine.execute("sudo tail -n 1 {}".format(log_file)),
             delay=5)
    ...
    def check_vm_force_reboot(self, vm_name, log_file):
        ...
        wait(lambda: re.match(r'[\s\S]+LOADPARM', self.machine.execute('sudo cat {}'.format(log_file))),
             delay=5)
