...
class MachinesDisksTestSuite(MachinesLib):
    ...
    def testDiskInfo(self):
        ...
        self.wait_css('#vm-{}-disks-vda-used'.format(name), cond=text_in, text_='0.00')
        self.wait_css('#vm-{}-disks-vda-capacity'.format(name), cond=text_in, text_='2')
    ...
    def testDetachDiskVmOn(self):
        ...
        self.click(self.wait_css('body > div:nth-child(2) > div.fade.in.modal > div > div > div.modal-footer > button.btn.btn-danger',
                                 cond=clickable))
        self.wait_dialog_disappear()
        self.wait_css('#delete-{}-disk-vdb'.format(name), cond=invisible)
        self.click(self.wait_css('#vm-{}-off'.format(name), cond=clickable))
        self.wait_css('#vm-{}-off'.format(name), cond=invisible)
        self.click(self.wait_css('#vm-{}-run'.format(name), cond=clickable))
        self.wait_css('#vm-{}-run'.format(name), cond=invisible)
    ...
    def testDetachDiskVmOff(self):
        ...
        self.click(self.wait_css('#delete-{}-disk-vdb'.format(name), cond=clickable))
        # click the double confirm button, then wait popup dialog disappear, or the invisible check will be out of action
        self.click(self.wait_css('body > div:nth-child(2) > div.fade.in.modal > div > div > div.modal-footer > button.btn.btn-danger',
                                 cond=clickable))
        self.wait_dialog_disappear()
        self.wait_css('#delete-{}-disk-vdb'.format(name), cond=invisible)
